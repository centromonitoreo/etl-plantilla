...
class EmailImplement(MailInformationServiceInterface):
    ...
    def create_mail_information(self, data: List[FeedDatabaseMail]) -> TableMailInformation:
        """
        ...
        for mail  in new_mail:
            email = TableMailInformationService().get_mail_information(mail.date_receipt)
            if not email:
            ...
            mail_id = email.id
            if mail.attachment_name is None: ############################ AQUI
                return None
            else:
                ...
                create_attachment.append(attachment)
